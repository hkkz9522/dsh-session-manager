...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      ".sm-footer{display:flex;width:100%}",
      ".sm-footerBtn{box-sizing:border-box;min-height:34px;width:calc(100% + 8px);margin:4px -4px;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border:0;border-radius:12px;align-items:center;gap:8px;padding:6px 2px 6px 10px;font-size:14px;line-height:22px;display:flex;overflow:hidden}",
      ".sm-footerBtn:hover,.sm-footerBtn:focus-visible{color:var(--dsw-alias-label-primary);background:var(--dsw-alias-interactive-bg-hover)}",
      ".sm-footerBtn svg{flex:none;display:block}",
    ...
    // ---------------------------------------------------------------------
    // Header actions (per open session): archive toggle + delete, as text
    // pills. Uses only primitives proven to render in the shipped shell
    // (Modal / IconArchiveOutline20 / IconWarningOutline16); no Menu, no
    // uncertain icons — React #130 (undefined element type) killed the old
    // icon/menu variant in the real browser.
    // ---------------------------------------------------------------------
    function HeaderAction({ sessionId, useSessions, useWorkspaces, t, onArchive, onUnarchive, onRemove }) {
      ...
      return h("div", { className: "sm-header" },
        h("button", {
          ...
          onClick: (e) => {
            e.stopPropagation();
            run(() => isArchived ? onUnarchive(sessionId) : onArchive(sessionId));
          }
        },
          h(P.IconArchiveOutline20, { size: 14 }),
          h("span", null, t(isArchived ? "menu.unarchive" : "menu.archive"))
        ),
        h("button", {
          type: "button",
          className: "sm-headerBtn sm-headerDanger",
          disabled: busy,
          title: t("menu.delete"),
          onClick: (e) => {
            e.stopPropagation();
    ...
    function SessionManagerPanel({ open, onClose, useSessions, useWorkspaces, t, onOpen, onArchive, onUnarchive, onRemove }) {
      ...
      const rows = useMemo(() => {
        // Hide subagent sessions and blank workspace placeholders (the
        // "New Session" seat DSH regenerates per workspace on reconnect) —
        // mirror the sidebar's sessionVisible rule: blank rows show only
        // when they are the current session.
        const all = list.ids
          .map((id) => list.byId[id])
          .filter((s) => s !== void 0 && s.origin !== "subagent" && (!s.blank || s.id === list.current));
