...
if (req.method === "GET" && path === "/preset-scan") {
    ...
    void (() => {
      ...
      let entries;
      try { entries = fss.readdirSync(root, { withFileTypes: true }); }
      catch { return; }
      let healedCount = 0;
      for (const proj of entries) {
        if (!proj.isDirectory()) continue;
        let dirs;
        try { dirs = fss.readdirSync(join(root, proj.name), { withFileTypes: true }); }
        catch { continue; }
        for (const entry of dirs) {
          if (!entry.isDirectory()) continue;
          ...
          try {
            const v2Stat = fss.statSync(v2Path);
            if (!v2Stat.isFile()) continue;
            try { fss.statSync(v0Path); continue; } catch { /* v0 doesn't exist, good */ }
            const buf = fss.readFileSync(v2Path);
            const text = zstdDecompressSync(buf).toString("utf8");
            const headerLine = text.split("\n", 1)[0];
            const header = JSON.parse(headerLine);
            if (header?.version !== 0) continue;
            fss.renameSync(v2Path, v0Path);
            healedCount += 1;
            ctx.logger.info(
